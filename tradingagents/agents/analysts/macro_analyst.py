...
def create_macro_analyst(llm, enable_web_search=True):
    ...
    def macro_analyst_node(state):
        ...
        system_message = (
            ...
            "- get_macro_causal_forecast(horizon_quarters, scenario, model, shocks, stock_beta): Run the quarterly causal Bayesian macro model (stability-constrained VARX + per-asset regressions, PyMC, fitted offline on FRED + market data) to get probabilistic forward scenarios for gold, S&P 500, Treasury bonds, and REITs: median cumulative returns, 25-75% and 10-90% intervals, P(positive), and tail risk. TWO query modes: (1) named scenarios — baseline, hawkish, inflation_shock, risk_off, productivity_boom over 1-12 quarters; (2) custom counterfactuals — pass shocks='variable:value' (variables: real_yield_chg, usd_ret, inflation_surprise, nfci_chg, profits_growth, mortgage_rate_chg; e.g. shocks='real_yield_chg:0.27' for the historical real-yield response to a 50bp hike, or 'usd_ret:1sd'); the shock and the asset return occur over the SAME quarter (conditional scenario, not an ex-ante forecast) and you MUST state the pass-through assumption you used. SCOPE: quarterly granularity only (no FOMC-day questions); for an individual stock set stock_beta to its beta vs the S&P 500 (e.g. stock_beta=1.0) and treat the beta-adjusted equity row as a linear single-factor surrogate without idiosyncratic risk. model='v1' is gold-only and one quarter ahead. INTERPRETATION: the signal is the DIFFERENCE vs baseline (levels embed bull-sample drift and are not price targets); quote probabilities and intervals, not point predictions. If the model is not fitted yet it returns setup instructions — fall back to the other tools in that case.\n"
            "- get_macro_sensitivity(): Show the fitted model's per-driver sensitivities — the next-quarter return response of gold, S&P 500, Treasuries, and REITs to a +1sd move in each macro driver (real yield, USD, oil, inflation surprise, mortgage rate, profit growth, NFCI), with 90% intervals. Call this FIRST when you want to know which channels matter for which asset, or to hand-translate a concrete move (e.g. +50bp hike ≈ +0.27pp real yield ≈ 0.9sd) into an approximate effect before running a full counterfactual.\n"
